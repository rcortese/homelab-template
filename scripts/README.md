...
- **Shared environment variables:** helpers accept variables such as `COMPOSE_INSTANCES`, `DOCKER_COMPOSE_BIN`, `APP_DATA_DIR`, `APP_DATA_DIR_MOUNT`, `APP_DATA_UID`/`APP_DATA_GID`, among others. See each section in [`docs/OPERATIONS.md`](../docs/OPERATIONS.md) for details and export them before execution when you need to customize behavior.
