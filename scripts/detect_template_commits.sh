...
first commit exclusive to the derived repository (FIRST_COMMIT_ID).

Options:
  --remote <name>         Remote that points to the source template.
  --target-branch <name>  Template branch used as reference.
  --output <file>         Path where the identifiers will be saved.
                          Default: env/local/template_commits.env
  --no-fetch              Do not run git fetch before calculating commits.
...
while [[ $# -gt 0 ]]; do
  ...
  -h | --help)
    ...
    ;;
  --remote)
    [[ $# -lt 2 ]] && error "--remote requires a value."
    remote="$2"
    shift 2
    ;;
  --target-branch)
    [[ $# -lt 2 ]] && error "--target-branch requires a value."
    target_branch="$2"
    shift 2
    ;;
  --output)
    [[ $# -lt 2 ]] && error "--output requires a value."
    output_file="$2"
    shift 2
    ;;
  --no-fetch)
...
[[ -n "$first_commit" ]] || error "no local-exclusive commit found after $original_commit."
