...
Automatically detects the template base commit (ORIGINAL_COMMIT_ID) and the
first commit unique to the derived repository (FIRST_COMMIT_ID).

Options:
  --remote <name>         Remote that points to the source template.
  --target-branch <name>  Template branch used as reference.
  --output <file>         Path where the identifiers file will be saved.
                          Default: env/local/template_commits.env
  --no-fetch              Skip git fetch before calculating.
...
[[ -n "$target_branch" ]] || error "unable to determine template branch. Use --target-branch."
...
[[ -n "$original_commit" ]] || error "unable to determine common ancestor between HEAD and $remote_ref."
