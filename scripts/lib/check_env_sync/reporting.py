"""Build and render sync reports for Compose manifests and env templates."""
...
def build_sync_report(repo_root: Path, metadata: ComposeMetadata) -> SyncReport:
    ...
    common_env_data = (
        load_env_variables(common_env_path)
        if common_env_path.exists()
        else empty_env_data
    )
    local_common_data = (
        load_env_variables(local_common_path)
    ...
    for path, data in instance_env_files.items():
        instance = next(
            (
                name
                for name, template in metadata.env_template_by_instance.items()
                if template == path
            ),
            None,
        )
