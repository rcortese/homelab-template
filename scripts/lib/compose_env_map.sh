...
load_env_map() {
  ...
  if ! declare -p COMPOSE_INSTANCE_FILES >/dev/null 2>&1 || ! declare -p COMPOSE_INSTANCE_NAMES >/dev/null 2>&1; then
    echo "[!] Metadados das instâncias não carregados. Execute load_compose_discovery primeiro." >&2
    return 1
  fi
