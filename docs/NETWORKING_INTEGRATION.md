...
| --- | --- | --- | --- |
| Reverse proxy | Platform team | TLS termination, hostname routing, security headers. | Receives public traffic → forwards to `compose/docker-compose.base.yml` (when present) + [`docker-compose.<instance>.yml`](../docker-compose.core.yml) *(replace with the active instance file)*. |
