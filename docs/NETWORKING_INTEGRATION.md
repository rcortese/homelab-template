...
| --- | --- | --- | --- |
| Reverse proxy | Platform team | TLS termination, hostname routing, security headers. | Receives public traffic → forwards to `compose/docker-compose.common.yml` (when present) + [`compose/docker-compose.<instance>.yml`](../compose/docker-compose.core.yml) *(replace with the active instance file)* + extra compose files (when present). |
