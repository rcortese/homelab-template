...
- [`scripts/bootstrap_instance.sh`](../scripts/bootstrap_instance.sh) with `--with-docs` creates these files from the [`doc-app.md.tpl`](../scripts/templates/bootstrap/doc-app.md.tpl) template and adds the corresponding link under the `## Applications` section of this index.
- After running the bootstrap, move the newly created item to the appropriate position in the list (for example, keeping alphabetical order) and revise the text whenever a new application is added or significantly changed.
