...
| --- | --- | --- |
| `compose/` | Base Docker Compose anchors and instance compose files. | `docker-compose.common.yml` (when necessary) plus `docker-compose.<instance>.yml` for each instance. |
...
| `compose/docker-compose.common.yml` | Optional | Shared services and shared resources (anchors, networks, volumes) loaded before any instance file. |
