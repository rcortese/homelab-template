...
- **Dicas de personalização para forks:**
  ...
  - Ajuste o `.env` da instância para apontar `APP_DATA_DIR` (relativo) ou `APP_DATA_DIR_MOUNT` (absoluto) quando o layout de dados divergir do padrão `data/<app>-<instância>` derivado automaticamente (via `APP_PRIMARY_APP` + `APP_INSTANCE`) — nunca habilite os dois ao mesmo tempo.
...
- **Diretórios persistentes:** o caminho `data/<app>-<instância>` é calculado automaticamente (com base em `APP_PRIMARY_APP` + `APP_INSTANCE`); utilize `APP_DATA_DIR` (relativo) **ou** `APP_DATA_DIR_MOUNT` (absoluto) quando precisar personalizar o destino e ajuste `APP_DATA_UID`/`APP_DATA_GID` no `.env` para alinhar permissões.
